        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        {availableSlots.map((slot) => (
                          <button key={slot} onClick={() => setBookingData({...bookingData, time: slot})}
                            className={`py-4 rounded-xl font-black text-[10px] tracking-widest transition-all active:scale-90 ${bookingData.time === slot ? 'bg-brand-pink text-white shadow-[0_0_15px_rgba(255,45,120,0.4)]' : 'bg-white/5 text-white/60 border border-white/10'}`}>
                            {slot}
                          </button>
                        ))}
                      </div>
          ...
          <div className="lg:col-span-4 h-fit sticky top-40 hidden lg:block">
             <div className="bg-white/5 border border-white/10 p-8 rounded-[2rem] space-y-6">
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-pink">Selection</h4>
                {selectedService && (
